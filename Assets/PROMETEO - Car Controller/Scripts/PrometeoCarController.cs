...
//   public void Brakes()
//   {
//     frontLeftCollider.brakeTorque = brakeForce * 0.7f;
//     frontRightCollider.brakeTorque = brakeForce * 0.7f;
//     rearLeftCollider.brakeTorque = brakeForce * 0.3f;
//     rearRightCollider.brakeTorque = brakeForce * 0.3f;
//   }
...
    //(MODIFIED) DRIFTING LOGIC
    // We handle the drifting state here, centrally. The car is only 'drifting' if the handbrake is held
    // and the car is actually sliding sideways. This will control the particle effects and sounds.
    bool handbrakeHeld = false;
    if (useTouchControls && touchControlsSetup)
    {
      handbrakeHeld = handbrakePTI.buttonPressed;
    }
    else
    {
    ...
    if (handbrakeHeld && Mathf.Abs(localVelocityX) > 2.5f)
    {
      isDrifting = true;
    }
    else
    ...
    // Update the particle effects based on the drifting state.
    DriftCarPS();
...
  public void GoForward()
  {
    //(REMOVED) The drift check was removed from here to centralize it in Update().
...
    isTractionLocked = true;
  ...
  public void DriftCarPS()
  {
    ...
    if (useEffects)
    {
      ...
      try
      {
        //(MODIFIED) Added a check to see if the rear wheels are on the ground before emitting the skid trails.
        bool isGrounded = rearLeftCollider.isGrounded || rearRightCollider.isGrounded;
